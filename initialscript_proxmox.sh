...
prefix_to_netmask(){ # 24 -> 255.255.255.0 (tanpa {1..4})
  local p="$1" m=() n i
  for i in 1 2 3 4; do
    if (( p>=8 )); then n=255
    elif (( p<=0 )); then n=0
...
must_ping(){ ping -c1 -W2 8.8.8.8 >/dev/null 2>&1; }

# ===== Pastikan pakai bash & tidak CRLF =====
if ! ps -p $$ -o comm= | grep -q bash; then
  die "Harus dijalankan dengan bash. Jalankan:  bash $0"
fi
if grep -q $'\r' "$0"; then
  warn "File tampak CRLF (Windows). Menormalkan ke UNIX..."
  sed -i 's/\r$//' "$0"
fi
...
GATE="$(detect_gw)"; [[ -n "${GATE:-}" ]] || die "Gateway tidak ditemukan."
WIRELESS="NO"; is_wireless "$MAIN_IF" && WIRELESS="YES"
log "Interface: $MAIN_IF (Wi-Fi? $WIRELESS)  IP: $IP4/$PREFIX  GW: $GATE"
if ! must_ping; then warn "Koneksi internet belum OK, lanjut tapi pastikan koneksi aktif."; fi
...
if [[ -f /etc/network/interfaces ]]; then
  BK="/etc/network/interfaces.bak.$(date +%s)"
  cp -a /etc/network/interfaces "$BK"
  log "Backup network: $BK"
fi
...
EOF
  ...
  sleep 2
  if ! must_ping; then
    warn "Internet host gagal setelah bridging. Rollback…"
    if [[ -n "${BK:-}" ]]; then cp -a "$BK" /etc/network/interfaces; fi
    systemctl restart networking || true
    die "Rollback selesai. Periksa nama NIC/gateway di file interfaces."
  fi
  log "Host online ✅ (bridged)."
...
EOF
  ...
  if ! must_ping; then
    ...
    if [[ -n "${BK:-}" ]]; then cp -a "$BK" /etc/network/interfaces; fi
    systemctl restart networking || true
    die "Rollback selesai. Cek log."
  fi
  log "Host online ✅ (Wi-Fi tetap aktif)."
  ...
  # DHCP untuk VM dengan dnsmasq (DHCP-only)
...
EOF
  ...
  if ip a show vmbr0 | grep -q "inet $VM_BR_IP"; then
    if dnsmasq --test; then
      systemctl enable --now dnsmasq
      log "dnsmasq aktif ✅ (range 192.168.100.50–200)"
    else
      warn "dnsmasq --test gagal; nonaktifkan dnsmasq. VM bisa pakai IP statis (192.168.100.50/24, GW $VM_BR_IP)."
      systemctl disable --now dnsmasq || true
    fi
  else
    warn "vmbr0 belum up; nonaktifkan dnsmasq. VM gunakan IP statis (GW $VM_BR_IP)."
    systemctl disable --now dnsmasq || true
  fi
fi
...
if [[ "$WIRELESS" == "NO" ]]; then
  echo "  Bridge       : vmbr0 (bridged ke $MAIN_IF) — VM dapat IP dari router rumah"
  echo "  WebUI        : https://$IP4:8006"
else
  echo "  Bridge       : vmbr0 ($VM_BR_IP/24, NAT → $MAIN_IF)"
  if systemctl is-enabled dnsmasq >/dev/null 2>&1; then
    echo "  DHCP VM      : dnsmasq aktif (192.168.100.50–200)"
  else
    echo "  DHCP VM      : nonaktif → set IP statis di VM (192.168.100.50/24, GW $VM_BR_IP, DNS 1.1.1.1)"
  fi
  echo "  WebUI        : https://$IP4:8006"
fi
warn "Jika WebUI belum bisa, coba reboot sekali:  reboot"